...
public class AttendanceData {
	...
	TreeMap<String,List<AttendanceData.AttendanceEvent>> eventsByDate;
	...
	AttendanceData (String html) {
		eventsByDate = new TreeMap<String, List<AttendanceData.AttendanceEvent>>();
		...
		pageUniqueId = Parser.pageUniqueId(doc);
	}
	
	public TreeMap<String, List<AttendanceData.AttendanceEvent>> getEventsByDate() {
			...
			for (Element attendanceDay : attendanceDays) {
				...
				for (int i = 1; i < periods.size(); i++) {
					...
					if (period.hasText()) {
						...
						AttendanceData.AttendanceEvent event = new AttendanceEvent(day, period.text(), classes.get(i-1));
						...
						List<AttendanceData.AttendanceEvent> eventsOnDay;
						if (eventsByDate.containsKey(day)) {
							eventsOnDay = eventsByDate.get(day);
						} else {
							eventsOnDay = new ArrayList<AttendanceData.AttendanceEvent>();
	...
	public void printDetailedView () {
		...
		for (int i = 0; i < eventsByPeriod.size(); i++) {
			...
			for (AttendanceData.AttendanceEvent e : eventsByPeriod.get(period)) {
		...
		for (Map.Entry<String,List<AttendanceData.AttendanceEvent>> entry : eventsByDate.entrySet()) {
			String date = entry.getKey();
			System.out.printf("%s:\n", date);
			for (AttendanceData.AttendanceEvent e : entry.getValue()) {
				System.out.println(e);
			}
		}
	}
